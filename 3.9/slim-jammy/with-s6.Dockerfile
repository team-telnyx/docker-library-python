...
##########################################################################
# Actual image.
FROM registry.internal.telnyx.com/jenkins/python:3.9-slim
...
RUN set -o nounset -o errexit -o xtrace -o verbose; \
    ...
    BASE_URL="https://github.com/just-containers/s6-overlay/releases/download" ; \
    if [ "${VERSION_S6_OVERLAY}" \< "3." ] ; then \
        curl --silent --show-error --location --output "${TARGET_PREFIX}-amd64.tar.gz" \
            "${BASE_URL}/v${VERSION_S6_OVERLAY}/s6-overlay-amd64.tar.gz" ; \
        grep "$TARGET_PREFIX" /opt/releases.sha512 | sha512sum -c - ; \
        tar --extract --gzip --file "${TARGET_PREFIX}-amd64.tar.gz" --directory / ; \
    else \
        curl --silent --show-error --location --output "${TARGET_PREFIX}-noarch.tar.xz" \
            "${BASE_URL}/v${VERSION_S6_OVERLAY}/s6-overlay-noarch.tar.xz" ; \
        curl --silent --show-error --location --output "${TARGET_PREFIX}-x86_64.tar.xz" \
            "${BASE_URL}/v${VERSION_S6_OVERLAY}/s6-overlay-x86_64.tar.xz" ; \
        grep "$TARGET_PREFIX" /opt/releases.sha512 | sha512sum -c - ; \
    ...
    ${PYBIN}/python3 -B -m pip install \
        ...
        --target=/opt/envtplpy/lib/python3.9/site-packages \
        --no-deps \
        --no-cache ; \
